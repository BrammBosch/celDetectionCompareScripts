...
def func(compare,dictArivis,listCounted,variatie,counted):
    ...
    score = abs(math.log10(counted / i)) * (abs(counted - i) + abs(arivis - counted))
    if score == 0:
        print('100% matched')
...
def makeGraph(dictArivis,counted,version,variatie,listCounted):
    ...
    legend_elements = [Patch(facecolor='b', edgecolor='r',alpha = 0.2,label='Not enough matches and not enough cells'),
                        ...
                        Patch(facecolor='r', edgecolor='r',alpha=0.2,label='Too many matches and too few cells'),
                        Patch(facecolor='y', edgecolor='r',alpha=0.2,label='Too few matches and too many cells')
                       ]
    ...
    point = ax.plot(refpoint, refpoint2, 'go', label='Perfect data\nx = ' + str(refpoint[0]) + '\ny = ' + str(refpoint[0]))
    ...
    ax.plot(listCoord1, listCoord2, 'g-', label='line')
